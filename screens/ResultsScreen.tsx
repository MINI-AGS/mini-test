...
import { View, Text, TouchableOpacity, Alert, ScrollView, StyleSheet, Dimensions, Platform} from 'react-native';
import { Timestamp } from 'firebase/firestore';
import { v4 as uuidv4 } from 'uuid';
import { getFirestore } from 'firebase/firestore';
import db from '../firebaseConfig';
import RecordFirestoreService from 'backend/RecordFirestoreService';
import { resultadoDiagnostico } from './QuestionsScreen/module';
import { myDiagnoses } from './QuestionsScreen/diagnosis';
import { questions } from './QuestionsScreen/questions';
...
const ResultsScreen = ({ route }: { route: any}) => {
  ...
  const construirRecord = (): any => {
    const birthdate = answers['birthdate'] ? new Date(answers['birthdate']) : null;
    const interviweDate = answers['interviewDate'] ? new Date(answers['interviewDate']) : null;

    const diagnosticosRelevantes = [
      "diagnosticA1", "diagnosticA2", "diagnosticA3",
      "diagnosticB1",
      "diagnosticC1", "riesgoC1",
      "diagnosticD1", "periodoD1", "diagnosticD2", "periodoD2",
      "diagnosticE1", "periodoE1", "diagnosticE2", "periodoE2", "diagnosticE3", "periodoE3",
      "diagnosticF1", "diagnosticF2", "diagnosticF3",
      "diagnosticG1",
      "diagnosticH1",
      "diagnosticI1",
      "diagnosticJ1", "diagnosticJ2",
      "diagnosticK1", "diagnosticK2",
      "diagnosticL1", "diagnosticL2", "diagnosticL3",
      "diagnosticM1",
      "diagnosticN1", "diagnosticN2",
      "diagnosticO1",
      "diagnosticP1"
    ];
    ...
    diagnosticosRelevantes.forEach((id) => {
      const diagnosis = myDiagnoses.find((d) => d.id === id);
      if (diagnosis) {
        const depende = diagnosis.dependsOn(answers);
        const resultado = diagnosis.result ? diagnosis.result(answers) : (depende ? "si" : "no");
        diagnosticosEvaluados[id] = typeof resultado === "string" ? resultado : (depende ? "si" : "no");

        const periodoId = `periodo${id.slice(-2)}`;
        if ((id.startsWith('diagnosticD') || id.startsWith('diagnosticE')) && diagnosis.result) {
    ...
    return {
      ...
      interviewDate: interviweDate ? Timestamp.fromDate(interviweDate) : null,
      ...
      questionK_Tranquilizantes_list: answers['questionK_Tranquilizantes_list'] || [""],
      questionK_Tranquilizantes_K2a: answers['questionK_Tranquilizantes_K2a'] || "", // son opcionales, solo si se escoge Tranquilizantes
      questionK_Tranquilizantes_K2b: answers['questionK_Tranquilizantes_K2b'] || "",
      questionK_Tranquilizantes_K2c: answers['questionK_Tranquilizantes_K2c'] || "",
      questionK_Tranquilizantes_K2d: answers['questionK_Tranquilizantes_K2d'] || "",
      questionK_Tranquilizantes_K2e: answers['questionK_Tranquilizantes_K2e'] || "",
      questionK_Tranquilizantes_K2f: answers['questionK_Tranquilizantes_K2f'] || "",
      questionK_Tranquilizantes_K3a: answers['questionK_Tranquilizantes_K3a'] || "",
      questionK_Tranquilizantes_K3b: answers['questionK_Tranquilizantes_K3b'] || "",
      questionK_Tranquilizantes_K3c: answers['questionK_Tranquilizantes_K3c'] || "",
      questionK_Tranquilizantes_K3d: answers['questionK_Tranquilizantes_K3d'] || "",
      
      // En caso que de escoger Otras Sustancias
      questionK_OtrasSustancias_list: answers['questionK_OtrasSustancias_list'] || [""],
      questionK_OtrasSustancias_K2a: answers['questionK_OtrasSustancias_K2a'] || "", // son opcionales, solo si se escoge Otras Sustancias
      questionK_OtrasSustancias_K2b: answers['questionK_OtrasSustancias_K2b'] || "",
      questionK_OtrasSustancias_K2c: answers['questionK_OtrasSustancias_K2c'] || "",
      questionK_OtrasSustancias_K2d: answers['questionK_OtrasSustancias_K2d'] || "",
      questionK_OtrasSustancias_K2e: answers['questionK_OtrasSustancias_K2e'] || "",
      questionK_OtrasSustancias_K2f: answers['questionK_OtrasSustancias_K2f'] || "",
      questionK_OtrasSustancias_K3a: answers['questionK_OtrasSustancias_K3a'] || "",
      questionK_OtrasSustancias_K3b: answers['questionK_OtrasSustancias_K3b'] || "",
      questionK_OtrasSustancias_K3c: answers['questionK_OtrasSustancias_K3c'] || "",
      questionK_OtrasSustancias_K3d: answers['questionK_OtrasSustancias_K3d'] || "",
  ...
  const guardarDatosEnFirebase = async () => {
    ...
    try {
      await service.createRecord(record.id, record);
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <TouchableOpacity style={[styles.button,Platform.OS === 'ios' ? styles.iosButton : styles.androidButton]} onPress={guardarDatosEnFirebase}>
